...
use App\Models\InventoryAdditionalInfo;
use App\Models\InventoryImageInfo;
    ...
    private function createTmpInventory($row)
    {
        // dd('certified' , $row[21]);
                ...
                $loan_amount = $payment_price - $down_payment;
                 ...
                 $monthly_payment = ceil(($loan_amount * $monthly_interest_rate) / (1 - pow(1 +
                     $monthly_interest_rate, -$calculateMonthValue)));
                ...
                switch ($row[7]) {
                    ...
                    $car_body = $row[7];
                        ...
                        $existingRecord = DealerInfo::where('user_id', auth()->user()->id)
                            ->where('dealer_name', $row[26])
                            ->first();
                        ...
                        if ($existingRecord) {
                            ...
                            $lastInsertedDealerId = $existingRecord->id; // Use the existing record's ID
                        } else {
                            // If no record exists, insert a new one
                            $lastInsertedDealerId = DealerInfo::insertGetId([
               ...
               $lastInsertedInventoryId = Inventory::insertGetId([
                                'dealer_info_id'                    => $lastInsertedDealerId,
                                'user_id'                           => auth()->user()->id,
                                'dealer_id'                         => $row[0],
                                'condition'                         => $row[1],
                                'stock'                             => $row[2],
                        ...
                        InventoryAdditionalInfo::create([
                            'inventory_id'                          => $lastInsertedInventoryId,
                            'comment_1'                             => $row[34],
                            'comment_2'                             => $row[35],
                            'comment_3'                             => $row[36],
                            'comment_4'                             => $row[37],
