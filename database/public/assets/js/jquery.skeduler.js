(function ($) {
    ...
    function appendTasks(placeholder, tasks) {
        ...
        tasks.forEach(function (task, index) {
            ...
            var width = task.width || 290;
            var left = task.left || 4;
            if (task.appointment_status == 0) {
                var color = '#003473';
            } else if (task.appointment_status == 1) {
                var color = '#4caf50';
            } else {
                var color = '#ffc107';
            }
            ...
            var card = $('<div></div>')
                .attr({
                    style: 'top: ' + top + 'px; height: ' + (height) + 'px; ' + 'width: ' + (width) + 'px; left: ' + left + 'px; background-color: ' + color + '!important'
                });
            card.on('click', function (e) {
                var url = window.location.protocol + '//' + window.location.host + '/admin/appointment-edit/' + task.appointment_id;
    ...
    $.fn.skeduler = function (options) {
        ...
        for (var j = 0; j < settings.headers.length; j++) {
            ...
            appendTasks(placeholder, settings.tasks.filter(function (t) {
                return t.column == j
            }));
